...
class LayerNorm(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # ==========================
        # la dernière dimension du tenseur 
        mean = inputs.mean(-1, keepdim=True)
        var = inputs.var(-1, keepdim=True, correction=0)
...
class MultiHeadedAttention(nn.Module):
    def __init__(self, head_size, num_heads, sequence_length=0):
        ...
        # TODO: Write your code here
        # ==========================
        self.input_dim_kqv_w = self.head_size * self.num_heads 
        self.output_dim_kqv_w = self.head_size * self.num_heads
        self.w_q = nn.Parameter(torch.empty(self.input_dim_kqv_w, self.output_dim_kqv_w))
        self.w_k = nn.Parameter(torch.empty(self.input_dim_kqv_w, self.output_dim_kqv_w))
        self.w_v = nn.Parameter(torch.empty(self.input_dim_kqv_w, self.output_dim_kqv_w))

        self.b_q = nn.Parameter(torch.empty(self.output_dim_kqv_w))
        self.b_k = nn.Parameter(torch.empty(self.output_dim_kqv_w))
        self.b_v = nn.Parameter(torch.empty(self.output_dim_kqv_w))

        self.w_y = nn.Parameter(torch.empty(self.head_size * self.num_heads, self.head_size * self.num_heads))
        self.b_y = nn.Parameter(torch.empty(self.head_size * self.num_heads))
    ...
    def get_attention_weights(self, queries, keys, mask=None):
        ...
        if mask is not None:
            attention_weights = attention_weights.masked_fill(mask.unsqueeze(1).unsqueeze(1) == 0, -1e9)
    ...
    def split_heads(self, tensor):
        ...
        # TODO: Write your code here
        # ==========================
        return tensor.view(tensor.size(0), self.sequence_length, self.num_heads, self.head_size).transpose(1, 2)
    ...
    def merge_heads(self, tensor):
        ...
        # TODO: Write your code here
        # ==========================
        return tensor.transpose(1, 2).contiguous().view(tensor.size(0), self.sequence_length, self.num_heads * self.head_size)
    ...
    def forward(self, hidden_states, mask=None):
        ...
        if self.sequence_length == 0:
            self.sequence_length = hidden_states.size(1)

        queries = self.split_heads(torch.matmul(hidden_states, self.w_q) + self.b_q)   
        keys = self.split_heads(torch.matmul(hidden_states, self.w_k) + self.b_k)
        values = self.split_heads(torch.matmul(hidden_states, self.w_v) + self.b_v)

        outputs = self.apply_attention(queries, keys, values, mask)
        outputs = torch.matmul(outputs, self.w_y) + self.b_y
        return outputs
...
class PreNormAttentionBlock(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        x_with_attention = x + attention_outputs
        normed_attention = self.layer_norm_2(x_with_attention)
        outputs = self.linear(normed_attention)

        return outputs + x_with_attention
